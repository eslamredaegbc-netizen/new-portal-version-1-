...
function renderDiagnostics(diagnostics) {
  const container = qs("provider-diagnostics");
  if (!Array.isArray(diagnostics) || !diagnostics.length) {
    container.innerHTML = '<div class="empty-state">لا توجد بيانات تشخيص بعد.</div>';
    return;
  }
  ...
  container.innerHTML = diagnostics
    .map(
      (item) => `
        <article class="diagnostic-card ${escapeHtml(item.status || "empty")}">
          <div class="card-head">
            <div>
              <h4 class="card-title">${escapeHtml(SOURCE_LABELS[item.source_type] || item.source_type || "مزود")}</h4>
              <div class="card-subtitle">${escapeHtml(item.query || "")}</div>
            </div>
            <div class="tag-row">
              ${pill(item.provider || "-", diagnosticTone(item.status))}
              ${pill(item.status || "-", diagnosticTone(item.status))}
              ${pill(`${formatNumber(item.count || 0)} نتيجة`, diagnosticTone(item.status))}
            </div>
          </div>
          <div class="detail-box">
            <strong>سبب الحالة</strong>
            <div>${escapeHtml(item.message || "لا توجد رسالة إضافية.")}</div>
          </div>
        </article>
      `
    )
...
function renderSearchOutcome(data) {
  ...
  state.selectedResultIds = new Set((data.results || []).map((item) => resultId(item)));
...
qs("search-form").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    qs("search-status").textContent =
      Number(data.results?.length || 0) > 0
        ? `تم العثور على ${formatNumber(data.results?.length || 0)} نتيجة قابلة للمراجعة والتحرير قبل التقرير.`
        : "لم يعثر النظام على نتائج قابلة للاعتماد في هذه المحاولة. راجع لوحة تشخيص التنفيذ لمعرفة حالة كل مزود بحث.";
    await loadDashboard();
  } catch (error) {
